...
- Fixed issues with fonts on distribute
...
- Added `Blackout Mode` to black out other monitors with borderless windows when fullscreen
  - When `BLACKOUT_MODE_TURN_OFF_MONITOR` is on in [config.h](./source/config.h) it turns the monitors off instead (experimental)
